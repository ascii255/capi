...
struct flag_enum_tag {};

template <typename T>
concept flag_enum = requires(T type) {
  { enable_flag_enum(type) } -> std::same_as<flag_enum_tag>;
};
...
capi::flag_enum_tag enable_flag_enum(test_flag);
capi::flag_enum_tag enable_flag_enum(signed_flag_enum);
